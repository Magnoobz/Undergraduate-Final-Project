...
int main()
{
    ...
    for (int i = 0; i < num_particle; i++)
    {
        if (pow(x[i]-0.5,2)+pow(y[i]-0.5,2)<pow(0.25,2))
        {
            split_index.push_back(i);
        }
    }
    ...
    for (int i = 0; i < num_particle; i++)
    {
        if (pow(x[i]-0.5,2)+pow(y[i]-0.5,2)<pow(0.15,2))
        {
            split_index.push_back(i);
        }
    }
    ...
    for (int i = 0; i < num_particle; i++)
    {
        if (pow(x[i]-0.5,2)+pow(y[i]-0.5,2)<pow(0.1,2))
        {
            split_index.push_back(i);
        }
    }
    ...
    while (loop_count < iter)
    {
        ...
        string name = "output/Test Multiresolusi/Tes Iterasi 7/Distribusi_" + to_string(nx) + "Partikel_" + to_string(loop_count) + "_iter.csv";
